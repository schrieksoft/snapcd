...
/// <summary>
/// Tier A smoke — AgentModuleAssignment is the representative entity for GenericAgentChildSecuredRepository.
/// </summary>
[Collection("NewRoleBasedSharedFixture")]
public class AgentModuleAssignment_SmokeTests : IAsyncLifetime
{
    private readonly Fixture _fixture;
    private SnapCdDbContext _dbContext = null!;

    public AgentModuleAssignment_SmokeTests(Fixture fixture) { _fixture = fixture; }
    ...
    public async Task Get_OwnerCanRead()
    {
        ...
        var entity = await repo.Get(_fixture.AgentModuleAssignments["0"].Id, _fixture.Organizations["0"].Id);
    ...
    public async Task Get_NoRoleCannotRead()
    {
        ...
        await Assert.ThrowsAsync<PrincipalNotAuthorizedException>(
            () => repo.Get(_fixture.AgentModuleAssignments["0"].Id, _fixture.Organizations["0"].Id));
    }
    ...
    public async Task List_OwnerSeesAssignment0()
    {
        ...
        Assert.Contains(items, a => a.Id == _fixture.AgentModuleAssignments["0"].Id);
    }
    ...
    {
        var target = _fixture.SmokeAgentAssignments["AgentModuleAssignment_SmokeTests_UpdateCan"];
        var repo = Repo(_fixture.OrganizationPrincipals["0"][OrganizationRole.Owner].DirectUser.Id);
    ...
    {
        var target = _fixture.AgentModuleAssignments["0"];
        var repo = Repo(_fixture.NoPermissionUser.Id);
    ...
    {
        var target = _fixture.SmokeAgentAssignments["AgentModuleAssignment_SmokeTests_DeleteCan"];
        var repo = Repo(_fixture.OrganizationPrincipals["0"][OrganizationRole.Owner].DirectUser.Id);
    ...
    public async Task Delete_NoRoleCannotDelete()
    {
        var repo = Repo(_fixture.NoPermissionUser.Id);
        await Assert.ThrowsAsync<PrincipalNotAuthorizedException>(
            () => repo.Delete(_fixture.AgentModuleAssignments["0"].Id, _fixture.Organizations["0"].Id));
    }

    private AgentModuleAssignmentSecuredRepository Repo(Guid principalId)
    {
        var pp = _fixture.CreatePrincipalProvider(principalId, PrincipalDiscriminator.User, _fixture.Organizations["0"].Id);
        return new AgentModuleAssignmentSecuredRepository(
