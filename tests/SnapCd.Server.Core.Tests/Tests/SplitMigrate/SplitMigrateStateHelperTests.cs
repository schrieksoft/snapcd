...
/// </summary>
public class SplitMigrateStateHelperTests
{
    [Fact]
    public void Every_Callback_Maps_To_A_State()
    {
        var unmapped = new List<SplitMigrateTaskEndpoint>();

        foreach (var endpoint in Enum.GetValues<SplitMigrateTaskEndpoint>())
        {
            try
            {
                SplitMigrateStateHelper.Lookup(endpoint);
            }
    ...
    {
        var machine = new SplitMigrateStateMachine(NullLogger<SplitMigrateStateMachine>.Instance);
        var declared = machine.States.Select(s => s.Name).ToHashSet();

        var missing = Enum.GetValues<SplitMigrateTaskEndpoint>()
            .Select(SplitMigrateStateHelper.Lookup)
            .Select(s => s.ToString())
            .Distinct()
    ...
    public void Cancelling_States_Exist_On_The_Machine()
    {
        ...
        var missing = SplitMigrateStateHelper.GetCancellingStates()
            .Select(s => s.ToString())
            .Where(s => !declared.Contains(s))
            .ToList();
