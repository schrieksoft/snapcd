...
public class SplitMigrateSagaRepositoryTests : IAsyncLifetime
{
    ...
    public SplitMigrateSagaRepositoryTests(Fixture fixture) => _fixture = fixture;
    ...
    public async Task DisposeAsync()
    {
        ...
        var sagas = await db.Set<SplitMigrateSaga>().Where(s => _created.Contains(s.CorrelationId)).ToListAsync();
        db.Set<SplitMigrateSaga>().RemoveRange(sagas);
    ...
    {
        var correlationId = await SeedSaga(SplitMigrateSagaState.MigrateProvePending.ToString());

        using var repository = Repo();
        var metaData = await repository.GetSagaMetaData(correlationId, _organizationId);

        Assert.Equal(SplitMigrateSagaState.MigrateProvePending.ToString(), metaData.CurrentState);
    ...
    public async Task Carries_The_Pinned_Runner_Instance()
    {
        var correlationId = await SeedSaga(
            SplitMigrateSagaState.MigrateRunPending.ToString(),
    ...
    {
        var correlationId = await SeedSaga(
            SplitMigrateSagaState.CancellingImmediateKill.ToString(),
            previousStateBeforeCancelling: SplitMigrateSagaState.MigrateMapPending.ToString());

        using var repository = Repo();
        var metaData = await repository.GetSagaMetaData(correlationId, _organizationId);

        Assert.Equal(SplitMigrateSagaState.MigrateMapPending.ToString(), metaData.PreviousStateBeforeCancelling);
    }
    ...
    {
        var correlationId = await SeedSaga(SplitMigrateSagaState.PlanPending.ToString());
    ...
    private SplitMigrateSagaRepository Repo() => new(_fixture.CreateDbContext());
