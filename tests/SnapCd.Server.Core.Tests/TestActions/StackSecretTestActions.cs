...
public class StackSecretTestActions : ITestActions
{
    private readonly Fixture _fixture;
    private readonly SnapCdDbContext _dbContext;

    public StackSecretTestActions(Fixture fixture, SnapCdDbContext dbContext)
    {
        _fixture = fixture;
    ...
    public async Task CanList(Guid principalId, PrincipalDiscriminator discriminator, Guid[] expectedEntityIds)
    {
        ...
        var repo = new StackSecretSecuredRepository(
            new StackSecretRepository(_dbContext, principalProvider, _fixture.CreateMockBus(),
                Microsoft.Extensions.Options.Options.Create(new StackSecretRepositorySettings())),
            principalProvider
        );
    ...
    {
        var entity = await _dbContext.StackSecrets.FirstOrDefaultAsync(s => s.Id == entityId);
    ...
    {
        var stack = await _dbContext.Stacks.FirstOrDefaultAsync(s => s.Id == parentId);
        var organizationId = stack?.OrganizationId ?? _fixture.Organizations["1"].Id;
