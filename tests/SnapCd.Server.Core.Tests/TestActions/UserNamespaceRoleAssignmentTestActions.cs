...
public class UserNamespaceRoleAssignmentTestActions : ITestActions
{
    private readonly Fixture _fixture;
    private readonly SnapCdDbContext _dbContext;

    public UserNamespaceRoleAssignmentTestActions(Fixture fixture, SnapCdDbContext dbContext)
    {
        _fixture = fixture;
    ...
    public async Task CanList(Guid principalId, PrincipalDiscriminator discriminator, Guid[] expectedEntityIds)
    {
        ...
        var repo = new UserNamespaceRoleAssignmentSecuredRepository(
            new UserNamespaceRoleAssignmentRepository(_dbContext, principalProvider, _fixture.CreateMockBus(),
                Microsoft.Extensions.Options.Options.Create(new UserNamespaceRoleAssignmentRepositorySettings())),
            principalProvider
        );
    ...
    {
        var entity = await _dbContext.UserNamespaceRoleAssignments.FirstOrDefaultAsync(a => a.Id == entityId);
    ...
    public async Task CanUpdate(Guid principalId, PrincipalDiscriminator discriminator, Guid entityId, string namePrefix)
    {
        ...
        var newRole = originalRole == NamespaceRole.Owner ? NamespaceRole.Contributor : NamespaceRole.Owner;
    ...
    {
        // parentId is the namespaceId for namespace role assignments
        var principalProvider = _fixture.CreatePrincipalProvider(principalId, discriminator, _fixture.Organizations["0"].Id);
        var repo = new UserNamespaceRoleAssignmentSecuredRepository(
            new UserNamespaceRoleAssignmentRepository(_dbContext, principalProvider, _fixture.CreateMockBus(),
                Microsoft.Extensions.Options.Options.Create(new UserNamespaceRoleAssignmentRepositorySettings())),
        ...
        var newAssignment = new UserNamespaceRoleAssignment
        {
            Id = Guid.NewGuid(),
    ...
    {
        // parentId is the namespaceId from Org "1" for cross-org isolation testing
        var principalProvider = _fixture.CreatePrincipalProvider(principalId, discriminator, _fixture.Organizations["0"].Id);
        var repo = new UserNamespaceRoleAssignmentSecuredRepository(
            new UserNamespaceRoleAssignmentRepository(_dbContext, principalProvider, _fixture.CreateMockBus(),
                Microsoft.Extensions.Options.Options.Create(new UserNamespaceRoleAssignmentRepositorySettings())),
