...
public class ServicePrincipalStackRoleAssignmentSecuredRepositoryFactory(
    IDbContextFactory<SnapCdDbContext> dbFactory,
    IPublishEndpoint bus,
    IOptions<ServicePrincipalStackRoleAssignmentRepositorySettings> options)
{
    public ServicePrincipalStackRoleAssignmentSecuredRepository Create(IPrincipalProvider? principalProvider = null)
    {
        if (principalProvider == null)
            principalProvider = new HttpContextPrincipalProvider(new HttpContextAccessor());
        var dbContext = dbFactory.CreateDbContext();
        return new ServicePrincipalStackRoleAssignmentSecuredRepository(
            new ServicePrincipalStackRoleAssignmentRepository(dbContext, principalProvider, bus, options),
            principalProvider);
    }
}

public class ServicePrincipalStackRoleAssignmentSecuredRepository : GenericStackChildSecuredRepository<
    ServicePrincipalStackRoleAssignment,
    ServicePrincipalStackRoleAssignmentReadDto,
    ServicePrincipalStackRoleAssignmentRepository,
    ServicePrincipalStackRoleAssignmentCreatedEvent,
    ServicePrincipalStackRoleAssignmentUpdatedEvent,
    ServicePrincipalStackRoleAssignmentDeletedEvent,
    ServicePrincipalStackRoleAssignmentRepositorySettings>
{
    public ServicePrincipalStackRoleAssignmentSecuredRepository(
        ServicePrincipalStackRoleAssignmentRepository repository,
