...
public class GroupNamespaceRoleAssignmentSecuredRepositoryFactory(
    IDbContextFactory<SnapCdDbContext> dbFactory,
    IPublishEndpoint bus,
    IOptions<GroupNamespaceRoleAssignmentRepositorySettings> options)
{
    public GroupNamespaceRoleAssignmentSecuredRepository Create(IPrincipalProvider? principalProvider = null)
    {
        if (principalProvider == null)
            principalProvider = new HttpContextPrincipalProvider(new HttpContextAccessor());
        var dbContext = dbFactory.CreateDbContext();
        return new GroupNamespaceRoleAssignmentSecuredRepository(
            new GroupNamespaceRoleAssignmentRepository(dbContext, principalProvider, bus, options),
            principalProvider);
    }
}

public class GroupNamespaceRoleAssignmentSecuredRepository : GenericNamespaceChildSecuredRepository<
    GroupNamespaceRoleAssignment,
    GroupNamespaceRoleAssignmentReadDto,
    GroupNamespaceRoleAssignmentRepository,
    GroupNamespaceRoleAssignmentCreatedEvent,
    GroupNamespaceRoleAssignmentUpdatedEvent,
    GroupNamespaceRoleAssignmentDeletedEvent,
    GroupNamespaceRoleAssignmentRepositorySettings>
{
    public GroupNamespaceRoleAssignmentSecuredRepository(
        GroupNamespaceRoleAssignmentRepository repository,
