...
public class ServicePrincipalModuleRoleAssignmentSecuredRepositoryFactory(
    IDbContextFactory<SnapCdDbContext> dbFactory,
    IPublishEndpoint bus,
    IOptions<ServicePrincipalModuleRoleAssignmentRepositorySettings> options)
{
    public ServicePrincipalModuleRoleAssignmentSecuredRepository Create(IPrincipalProvider? principalProvider = null)
    {
        if (principalProvider == null)
            principalProvider = new HttpContextPrincipalProvider(new HttpContextAccessor());
        var dbContext = dbFactory.CreateDbContext();
        return new ServicePrincipalModuleRoleAssignmentSecuredRepository(
            new ServicePrincipalModuleRoleAssignmentRepository(dbContext, principalProvider, bus, options),
            principalProvider);
    }
}

public class ServicePrincipalModuleRoleAssignmentSecuredRepository : GenericModuleChildSecuredRepository<
    ServicePrincipalModuleRoleAssignment,
    ServicePrincipalModuleRoleAssignmentReadDto,
    ServicePrincipalModuleRoleAssignmentRepository,
    ServicePrincipalModuleRoleAssignmentCreatedEvent,
    ServicePrincipalModuleRoleAssignmentUpdatedEvent,
    ServicePrincipalModuleRoleAssignmentDeletedEvent,
    ServicePrincipalModuleRoleAssignmentRepositorySettings>
{
    public ServicePrincipalModuleRoleAssignmentSecuredRepository(
        ServicePrincipalModuleRoleAssignmentRepository repository,
