...
public class StateStoreRoleAssignmentSecuredRepositoryFactory(
    IDbContextFactory<SnapCdDbContext> dbFactory,
    IPublishEndpoint bus,
    IOptions<StateStoreRoleAssignmentRepositorySettings> options)
{
    public StateStoreRoleAssignmentSecuredRepository Create(IPrincipalProvider? principalProvider = null)
    {
        if (principalProvider == null)
            principalProvider = new HttpContextPrincipalProvider(new HttpContextAccessor());
        var dbContext = dbFactory.CreateDbContext();
        return new StateStoreRoleAssignmentSecuredRepository(
            new StateStoreRoleAssignmentRepository(dbContext, principalProvider, bus, options),
            principalProvider);
    }
}
...
public class StateStoreRoleAssignmentSecuredRepository : GenericStateStoreChildSecuredRepository<
    ...
    StateStoreRoleAssignmentCreatedEvent,
    StateStoreRoleAssignmentUpdatedEvent,
    StateStoreRoleAssignmentDeletedEvent,
    StateStoreRoleAssignmentRepositorySettings>
{
    public StateStoreRoleAssignmentSecuredRepository(
        StateStoreRoleAssignmentRepository repository,
    ...
    public override async Task<StateStoreRoleAssignment> Create(StateStoreRoleAssignment entity, bool inTransaction = true)
    {
        throw new NotImplementedByDesignException(
            "StateStoreRoleAssignmentSecuredRepository can only be used for Get, List and Delete requests. For all others, use a repository for a concrete class.");
    }

    public override async Task<StateStoreRoleAssignment> Update(StateStoreRoleAssignment entity, bool inTransaction = true)
    {
        throw new NotImplementedByDesignException(
            "StateStoreRoleAssignmentSecuredRepository can only be used for Get, List and Delete requests. For all others, use a repository for a concrete class.");
    }
