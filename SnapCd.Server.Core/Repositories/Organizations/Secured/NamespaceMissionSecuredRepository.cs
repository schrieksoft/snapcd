...
public class NamespaceMissionSecuredRepositoryFactory(
    IDbContextFactory<SnapCdDbContext> dbFactory,
    IPublishEndpoint bus,
    IOptions<NamespaceMissionRepositorySettings> options)
{
    public NamespaceMissionSecuredRepository Create(IPrincipalProvider? principalProvider = null)
    {
        if (principalProvider == null)
            principalProvider = new HttpContextPrincipalProvider(new HttpContextAccessor());
        var dbContext = dbFactory.CreateDbContext();
        return new NamespaceMissionSecuredRepository(
            new NamespaceMissionRepository(dbContext, principalProvider, bus, options),
            principalProvider);
    }
}

public class NamespaceMissionSecuredRepository : GenericNamespaceChildSecuredRepository<
    NamespaceMission,
...
{
    public NamespaceMissionSecuredRepository(
        NamespaceMissionRepository repository,
    ...
    public override IQueryable<NamespaceMission> ReadQuery(Guid organizationId)
    {
        ...
        if (PrincipalDiscriminator == PrincipalDiscriminator.User)
        {
            var directAgentQuery =
                from m in ctx.NamespaceMissions
