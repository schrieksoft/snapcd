...
public class AgentStackAssignmentSecuredRepositoryFactory(
    IDbContextFactory<SnapCdDbContext> dbFactory,
    IPublishEndpoint bus,
    IOptions<AgentStackAssignmentRepositorySettings> options)
{
    public AgentStackAssignmentSecuredRepository Create(IPrincipalProvider? principalProvider = null)
    {
        if (principalProvider == null)
            principalProvider = new HttpContextPrincipalProvider(new HttpContextAccessor());
        var dbContext = dbFactory.CreateDbContext();
        return new AgentStackAssignmentSecuredRepository(
            new AgentStackAssignmentRepository(dbContext, principalProvider, bus, options),
            principalProvider);
    }
}

public class AgentStackAssignmentSecuredRepository : GenericAgentChildSecuredRepository<
...
{
    public AgentStackAssignmentSecuredRepository(
        AgentStackAssignmentRepository repository,
