...
public class GroupStateStoreRoleAssignmentRepositoryFactory(IDbContextFactory<SnapCdDbContext> dbFactory, IPublishEndpoint bus, IOptions<GroupStateStoreRoleAssignmentRepositorySettings> options)
{
    public GroupStateStoreRoleAssignmentRepository Create(IPrincipalProvider? principalProvider = null)
    {
        if (principalProvider == null)
            principalProvider = new HttpContextPrincipalProvider(new HttpContextAccessor());
        var dbContext = dbFactory.CreateDbContext();
        return new GroupStateStoreRoleAssignmentRepository(dbContext, principalProvider, bus, options);
    }
}
    ...
    GroupStateStoreRoleAssignmentUpdatedEvent, GroupStateStoreRoleAssignmentDeletedEvent, GroupStateStoreRoleAssignmentRepositorySettings>
{
    ...
    protected override GroupStateStoreRoleAssignmentReadDto MapToDto(GroupStateStoreRoleAssignment entity)
    {
        return GroupStateStoreRoleAssignmentMapper.ToDto(entity);
    }

    protected override async Task<QuotaCheckResult> CheckQuotaAsync(GroupStateStoreRoleAssignment entity)
    {
        var currentCount = await DbContext.GroupStateStoreRoleAssignments
            .CountAsync(e => e.OrganizationId == entity.OrganizationId);

        return await CheckQuotaWithServiceAsync(entity.OrganizationId, nameof(Settings.QuotaLimits.GroupStateStoreRoleAssignmentQuota), currentCount);
    }

    public async Task<List<GroupStateStoreRoleAssignment>> ListByGroup(Guid groupId, Guid organizationId)
    {
        return await DbContext.GroupStateStoreRoleAssignments
