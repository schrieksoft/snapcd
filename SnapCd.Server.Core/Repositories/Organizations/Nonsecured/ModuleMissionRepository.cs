...
public class ModuleMissionRepositoryFactory(IDbContextFactory<SnapCdDbContext> dbFactory, IPublishEndpoint bus, IOptions<ModuleMissionRepositorySettings> options)
{
    public ModuleMissionRepository Create(IPrincipalProvider? principalProvider = null)
    {
        if (principalProvider == null)
            principalProvider = new HttpContextPrincipalProvider(new HttpContextAccessor());
        var dbContext = dbFactory.CreateDbContext();
        return new ModuleMissionRepository(dbContext, principalProvider, bus, options);
    }
}
...
public class ModuleMissionRepository : GenericModuleChildRepository<ModuleMission, ModuleMissionReadDto, ModuleMissionCreatedEvent, ModuleMissionUpdatedEvent, ModuleMissionDeletedEvent, ModuleMissionRepositorySettings>
{
    ...
    protected override async Task<QuotaCheckResult> CheckQuotaAsync(ModuleMission entity)
    {
        var currentCount = await DbContext.ModuleMissions
            .CountAsync(e => e.OrganizationId == entity.OrganizationId);

        return await CheckQuotaWithServiceAsync(entity.OrganizationId, nameof(Settings.QuotaLimits.ModuleMissionQuota), currentCount);
    }

    public async Task<List<ModuleMission>> ListByAgent(Guid agentId, Guid organizationId)
    {
        return await DbContext.ModuleMissions
    ...
    {
        return await DbContext.ModuleMissions
            .Where(m => m.OrganizationId == organizationId && m.ModuleId == moduleId)
            .ToListAsync();
    }
}
