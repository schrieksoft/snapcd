...
public class RunnerNamespaceSupplyRepositoryFactory(IDbContextFactory<SnapCdDbContext> dbFactory, IPublishEndpoint bus, IOptions<RunnerNamespaceSupplyRepositorySettings> options)
{
    public RunnerNamespaceSupplyRepository Create(IPrincipalProvider? principalProvider = null)
    {
        if (principalProvider == null)
            principalProvider = new HttpContextPrincipalProvider(new HttpContextAccessor());
        var dbContext = dbFactory.CreateDbContext();
        return new RunnerNamespaceSupplyRepository(dbContext, principalProvider, bus, options);
    }
}
    ...
    RunnerNamespaceSupplyUpdatedEvent, RunnerNamespaceSupplyDeletedEvent, RunnerNamespaceSupplyRepositorySettings>
{
    ...
    protected override async Task<QuotaCheckResult> CheckQuotaAsync(RunnerNamespaceSupply entity)
    {
        var currentCount = await DbContext.RunnerNamespaceSupplies
            .CountAsync(e => e.OrganizationId == entity.OrganizationId);

        return await CheckQuotaWithServiceAsync(entity.OrganizationId, nameof(Settings.QuotaLimits.RunnerNamespaceSupplyQuota), currentCount);
    }

    public async Task<List<RunnerNamespaceSupply>> ListByRunner(Guid runnerId, Guid organizationId)
    {
        return await DbContext.RunnerNamespaceSupplies
