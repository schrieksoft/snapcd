...
public class NamespaceTerraformLocalPolicyRepositoryFactory(IDbContextFactory<SnapCdDbContext> dbFactory, IPublishEndpoint bus, IOptions<NamespaceTerraformLocalPolicyRepositorySettings> options)
{
    public NamespaceTerraformLocalPolicyRepository Create(IPrincipalProvider? principalProvider = null)
    {
        if (principalProvider == null)
            principalProvider = new HttpContextPrincipalProvider(new HttpContextAccessor());
        var dbContext = dbFactory.CreateDbContext();
        return new NamespaceTerraformLocalPolicyRepository(dbContext, principalProvider, bus, options);
    }
}
    ...
    NamespaceTerraformLocalPolicyDeletedEvent, NamespaceTerraformLocalPolicyRepositorySettings>
{
    ...
    protected override async Task<QuotaCheckResult> CheckQuotaAsync(NamespaceTerraformLocalPolicy entity)
    {
        var currentCount = await DbContext.NamespaceTerraformLocalPolicies
            .CountAsync(e => e.OrganizationId == entity.OrganizationId);

        return await CheckQuotaWithServiceAsync(entity.OrganizationId, nameof(Settings.QuotaLimits.NamespaceTerraformLocalPolicyQuota), currentCount);
    }

    public async Task<NamespaceTerraformLocalPolicy> Get(Guid namespaceId, string name, Guid organizationId)
    {
        var entity = await DbContext.NamespaceTerraformLocalPolicies
            .SingleOrDefaultAsync(e => e.Name == name && e.NamespaceId == namespaceId && e.OrganizationId == organizationId);

        if (entity == null)
            throw new EntityNotFoundException($"NamespaceTerraformLocalPolicy with name {name} not found for namespace {namespaceId}.");
