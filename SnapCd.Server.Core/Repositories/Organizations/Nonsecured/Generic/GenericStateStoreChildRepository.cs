...
{
    public GenericStateStoreChildRepository(SnapCdDbContext dbContext, IPrincipalProvider principalProvider, IPublishEndpoint bus, IOptions<TSettings> options)
        : base(dbContext, principalProvider, bus, options)
    {
    }

    protected override Func<IQueryable<TEntity>, IQueryable<TEntity>> ByParentIdQueryModifier(Guid stateStoreId)
    {
        return query => query.Where(e => e.StateStoreId == stateStoreId);
    }
}
