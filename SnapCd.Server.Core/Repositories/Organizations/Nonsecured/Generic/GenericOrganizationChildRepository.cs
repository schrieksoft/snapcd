...
public abstract class GenericOrganizationChildRepository<TEntity, TDto, TCreateEvent, TUpdateEvent, TDeleteEvent, TSettings> :
    ...
    where TSettings : class, IEntitySettings
{
    ...
    protected override Func<IQueryable<TEntity>, IQueryable<TEntity>> ByParentIdQueryModifier(Guid organizationId)
    {
        return query => query.Where(e => e.OrganizationId == organizationId);
    }
}