...
/// <summary>
/// Drives a SplitMigrate manual job. Separate from JobStateMachine because that machine's shape is
...
/// </summary>
public partial class SplitMigrateStateMachine : MassTransitStateMachine<SplitMigrateSaga>
{
    private readonly ILogger<SplitMigrateStateMachine> _logger;

    // Job-level entry point
    public Event<SplitMigrateRequested> SplitRequested { get; } = null!;

    // Liveness
    public Event<RunnerReconnectedEvent> RunnerReconnectedEvent { get; } = null!;
    public Request<SplitMigrateSaga, HeartbeatRequested, HeartbeatCompleted, HeartbeatFailed> HeartbeatRequested { get; } = null!;
    public Schedule<SplitMigrateSaga, HeartbeatScheduled> HeartbeatScheduled { get; } = null!;

    // Terminal
    public State Completed { get; } = null!;
    public State Failed { get; } = null!;

    public SplitMigrateStateMachine(ILogger<SplitMigrateStateMachine> logger)
    {
        _logger = logger;
