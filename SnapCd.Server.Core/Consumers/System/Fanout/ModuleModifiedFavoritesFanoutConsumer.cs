...
/// <summary>
/// Refreshes the nav menu for users who starred this module. Favorite links carry its name
/// in their label and href, and resolve only when the menu loads.
/// </summary>
public class ModuleModifiedFavoritesFanoutConsumer : IConsumer<ModuleModifiedEvent>
{
    private readonly SnapCdDbContext _dbContext;
    private readonly FavoritesModifiedNotificationService _notificationService;

    public ModuleModifiedFavoritesFanoutConsumer(
    ...
    public async Task Consume(ConsumeContext<ModuleModifiedEvent> context)
    {
        var userIds = await _dbContext.UserFavorites
            .Where(f => f.TargetType == FavoriteTargetType.Module && f.TargetId == context.Message.Id)
            .Select(f => f.UserId)
            .Distinct()
