...
/// (so a new <c>AgentNamespaceSupply</c> wakes a parked <c>ModuleMission</c> only if that module is in
/// the relevant namespace — the dispatcher's <see cref="AgentSupplyResolver"/> evaluates the OR-chain).
/// Also handles <see cref="AgentUpdatedEvent"/> for the <c>IsSuppliedToAllModules</c> flag flipping to
/// true. Competing — one instance per event handles all the parked missions for the affected agent.
/// </summary>
public class AgentSupplyCreatedMissionWakeConsumer :
    IConsumer<AgentStackSupplyCreatedEvent>,
    IConsumer<AgentNamespaceSupplyCreatedEvent>,
    IConsumer<AgentModuleSupplyCreatedEvent>,
    IConsumer<AgentUpdatedEvent>
{
    private readonly IDbContextFactory<SnapCdDbContext> _dbContextFactory;
    private readonly MissionDispatcher _dispatcher;
    private readonly ILogger<AgentSupplyCreatedMissionWakeConsumer> _logger;

    public AgentSupplyCreatedMissionWakeConsumer(
        IDbContextFactory<SnapCdDbContext> dbContextFactory,
        MissionDispatcher dispatcher,
        ILogger<AgentSupplyCreatedMissionWakeConsumer> logger)
    {
    ...
    public Task Consume(ConsumeContext<AgentStackSupplyCreatedEvent> context) =>
        WakeAsync(context.Message.Data.AgentId, context.Message.OrganizationId, context.CancellationToken);

    public Task Consume(ConsumeContext<AgentNamespaceSupplyCreatedEvent> context) =>
        WakeAsync(context.Message.Data.AgentId, context.Message.OrganizationId, context.CancellationToken);

    public Task Consume(ConsumeContext<AgentModuleSupplyCreatedEvent> context) =>
        WakeAsync(context.Message.Data.AgentId, context.Message.OrganizationId, context.CancellationToken);
    ...
    public Task Consume(ConsumeContext<AgentUpdatedEvent> context)
    {
        ...
        return context.Message.Data.IsSuppliedToAllModules
