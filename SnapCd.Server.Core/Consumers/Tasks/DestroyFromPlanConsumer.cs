...
/// <summary>
/// Server-side consumer that receives DestroyFromPlan requests and dispatches them to runners via SignalR.
/// Replaces the old runner-side consumer pattern with direct hub invocation.
/// </summary>
public class DestroyFromPlanConsumer : IConsumer<DestroyFromPlanRequested>
{
    private readonly ILogger<DestroyFromPlanConsumer> _logger;
    private readonly IHubContext<RunnerHub> _hubContext;
    private readonly RunnerSelectionService _runnerSelection;

    public DestroyFromPlanConsumer(
        ILogger<DestroyFromPlanConsumer> logger,
    ...
    public async Task Consume(ConsumeContext<DestroyFromPlanRequested> context)
    {
        ...
        try
        {
            ...
            await _hubContext.Clients.Client(runner.SignalRConnectionId).SendAsync(
                ...
                new DestroyFromPlanRequestBase
                {
                    ...
                    DestroyBeforeHook = msg.Declared.DestroyBeforeHook,
                    DestroyAfterHook = msg.Declared.DestroyAfterHook,
                    ...
                    TerraformFlags = msg.Declared.TerraformFlags
                        .Where(f => f.Task == TerraformCommandTask.Destroy)
                        .ToList(),
        ...
        {
            _logger.LogError(ex, "Error dispatching DestroyFromPlan request for job {JobId}", jobId);
            await context.Publish(new DestroyFromPlanFaulted
            {
                CorrelationId = jobId,
                OrganizationId = orgId,
