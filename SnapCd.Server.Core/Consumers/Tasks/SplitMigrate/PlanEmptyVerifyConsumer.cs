...
namespace SnapCd.Server.Core.Consumers.Tasks.SplitMigrate;

public class PlanEmptyVerifyConsumer : IConsumer<PlanEmptyVerifyRequested>
{
    private readonly ILogger<PlanEmptyVerifyConsumer> _logger;
    private readonly IHubContext<RunnerHub> _hubContext;
    private readonly RunnerSelectionService _runnerSelection;

    public PlanEmptyVerifyConsumer(
        ILogger<PlanEmptyVerifyConsumer> logger,
    ...
    public async Task Consume(ConsumeContext<PlanEmptyVerifyRequested> context)
    {
        ...
        try
        {
            ...
            await _hubContext.Clients.Client(runner.SignalRConnectionId).SendAsync(
                RunnerEndpoints.PlanEmptyVerify,
                new PlanEmptyVerifyRequestBase
                {
                    JobId = jobId,
            ...
            _logger.LogDebug("Dispatched PlanEmptyVerify request to runner {RunnerName} for job {JobId}",
                runner.InstanceName, jobId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error dispatching PlanEmptyVerify request for job {JobId}", jobId);
            await context.Publish(new PlanEmptyVerifyFaulted
            {
                CorrelationId = jobId,
                OrganizationId = orgId,
