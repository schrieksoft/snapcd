...
/// <summary>Dispatches a split job's GetModule step. The payload is the ordinary one; only the contract differs.</summary>
public class SplitGetModuleConsumer : IConsumer<SplitGetModuleRequested>
{
    private readonly ILogger<SplitGetModuleConsumer> _logger;
    private readonly IHubContext<RunnerHub> _hubContext;
    private readonly RunnerSelectionService _runnerSelection;

    public SplitGetModuleConsumer(
        ILogger<SplitGetModuleConsumer> logger,
    ...
    public async Task Consume(ConsumeContext<SplitGetModuleRequested> context)
    {
        ...
        {
            _logger.LogError(ex, "Error dispatching SplitGetModule request for job {JobId}", jobId);
            await context.Publish(new SplitGetModuleFaulted
            {
                CorrelationId = jobId,
