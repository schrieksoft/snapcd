...
public class MigrateVerifyConsumer : IConsumer<MigrateVerifyRequested>
{
    private readonly ILogger<MigrateVerifyConsumer> _logger;
    private readonly IHubContext<RunnerHub> _hubContext;
    private readonly RunnerSelectionService _runnerSelection;

    public MigrateVerifyConsumer(
        ILogger<MigrateVerifyConsumer> logger,
    ...
    public async Task Consume(ConsumeContext<MigrateVerifyRequested> context)
    {
        ...
        try
        {
            ...
            await _hubContext.Clients.Client(runner.SignalRConnectionId).SendAsync(
                RunnerEndpoints.MigrateVerify,
                new MigrateVerifyRequestBase
                {
                    JobId = jobId,
            ...
            _logger.LogDebug("Dispatched MigrateVerify request to runner {RunnerName} for job {JobId}",
                runner.InstanceName, jobId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error dispatching MigrateVerify request for job {JobId}", jobId);
            await context.Publish(new MigrateVerifyFaulted
            {
                CorrelationId = jobId,
                OrganizationId = orgId,
