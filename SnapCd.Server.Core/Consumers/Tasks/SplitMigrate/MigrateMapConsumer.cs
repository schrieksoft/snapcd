...
public class MigrateMapConsumer : IConsumer<MigrateMapRequested>
{
    private readonly ILogger<MigrateMapConsumer> _logger;
    private readonly IHubContext<RunnerHub> _hubContext;
    private readonly RunnerSelectionService _runnerSelection;

    public MigrateMapConsumer(
        ILogger<MigrateMapConsumer> logger,
    ...
    public async Task Consume(ConsumeContext<MigrateMapRequested> context)
    {
        ...
        try
        {
            ...
            await _hubContext.Clients.Client(runner.SignalRConnectionId).SendAsync(
                RunnerEndpoints.MigrateMap,
                new MigrateMapRequestBase
                {
                    JobId = jobId,
            ...
            _logger.LogDebug("Dispatched MigrateMap request to runner {RunnerName} for job {JobId}",
                runner.InstanceName, jobId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error dispatching MigrateMap request for job {JobId}", jobId);
            await context.Publish(new MigrateMapFaulted
            {
                CorrelationId = jobId,
                OrganizationId = orgId,
