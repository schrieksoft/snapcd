...
/// <summary>
/// Server-side consumer that receives ApplyFromPlan requests and dispatches them to runners via SignalR.
/// Replaces the old runner-side consumer pattern with direct hub invocation.
/// </summary>
public class ApplyFromPlanConsumer : IConsumer<ApplyFromPlanRequested>
{
    private readonly ILogger<ApplyFromPlanConsumer> _logger;
    private readonly IHubContext<RunnerHub> _hubContext;
    private readonly RunnerSelectionService _runnerSelection;

    public ApplyFromPlanConsumer(
        ILogger<ApplyFromPlanConsumer> logger,
    ...
    public async Task Consume(ConsumeContext<ApplyFromPlanRequested> context)
    {
        ...
        _logger.LogInformation("Received ApplyFromPlan request for job {JobId} in pool {RunnerId}",
            jobId, runnerId);
        ...
        try
        {
            ...
            await _hubContext.Clients.Client(runner.SignalRConnectionId).SendAsync(
                RunnerEndpoints.ApplyFromPlan,
                new ApplyFromPlanRequestBase
                {
                    JobId = jobId,
                    ...
                    ApplyBeforeHook = msg.Declared.ApplyBeforeHook,
                    ApplyAfterHook = msg.Declared.ApplyAfterHook,
                    PulumiFlags = msg.Declared.PulumiFlags
                        .Where(f => f.Task == PulumiCommandTask.Apply)
                        .ToList(),
                    PulumiArrayFlags = msg.Declared.PulumiArrayFlags
                        .Where(f => f.Task == PulumiCommandTask.Apply)
                        .ToList(),
                    TerraformFlags = msg.Declared.TerraformFlags
                        .Where(f => f.Task == TerraformCommandTask.Apply)
                        .ToList(),
            ...
            _logger.LogInformation("Dispatched ApplyFromPlan request to runner {RunnerName} for job {JobId}",
                runner.InstanceName, jobId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error dispatching ApplyFromPlan request for job {JobId}", jobId);
            await context.Publish(new ApplyFromPlanFaulted
            {
                CorrelationId = jobId,
                OrganizationId = orgId,
