...
public class ModuleInputFromSecretBaseService
{
    private readonly ModuleInputSecuredRepository _repo;

    public ModuleInputFromSecretBaseService(
        ModuleInputSecuredRepository repo)
    {
        _repo = repo;
    }

    public async Task<ModuleInputFromSecretReadDto> Get(Guid id, Guid organizationId)
    {
        var entity = await _repo.Get(id, organizationId);
        var dto = entity switch
        {
            ModuleParamFromSecret param => ModuleInputFromSecretMapper.ToDto(param),
            ModuleEnvVarFromSecret envVar => ModuleInputFromSecretMapper.ToDto(envVar),
            _ => throw new InvalidOperationException($"Unknown entity type: {entity.GetType().Name}")
        };

        dto.InputKind = entity switch
        {
            ModuleParamFromSecret => InputKind.Param,
            ModuleEnvVarFromSecret => InputKind.EnvVar,
            _ => throw new InvalidOperationException($"Unknown entity type: {entity.GetType().Name}")
        };
