...
public class NamespaceInputFromDefinitionBaseService
{
    private readonly NamespaceInputSecuredRepository _repo;

    public NamespaceInputFromDefinitionBaseService(
        NamespaceInputSecuredRepository repo)
    {
        _repo = repo;
    }

    public async Task<NamespaceInputFromDefinitionReadDto> Get(Guid id, Guid organizationId)
    {
        var entity = await _repo.Get(id, organizationId);
        var dto = entity switch
        {
            NamespaceParamFromDefinition param => NamespaceInputFromDefinitionMapper.ToDto(param),
            NamespaceEnvVarFromDefinition envVar => NamespaceInputFromDefinitionMapper.ToDto(envVar),
            _ => throw new InvalidOperationException($"Unknown entity type: {entity.GetType().Name}")
        };

        dto.InputKind = entity switch
        {
            NamespaceParamFromDefinition => InputKind.Param,
            NamespaceEnvVarFromDefinition => InputKind.EnvVar,
            _ => throw new InvalidOperationException($"Unknown entity type: {entity.GetType().Name}")
        };
