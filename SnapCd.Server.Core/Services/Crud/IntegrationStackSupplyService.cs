...
{
    public IntegrationStackSupplyService(
        IntegrationStackSupplySecuredRepository securedRepository) : base(securedRepository)
    {
    }

    protected override IntegrationStackSupply MapToEntity(IntegrationStackSupplyCreateDto dto, Guid organizationId)
    {
        return IntegrationStackSupplyMapper.ToEntity(dto, organizationId);
    }
    ...
    {
        IntegrationStackSupplyMapper.UpdateEntity(entity, dto);
    }
}
