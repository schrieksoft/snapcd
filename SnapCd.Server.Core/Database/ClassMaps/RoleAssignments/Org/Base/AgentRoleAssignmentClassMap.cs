...
public class AgentRoleAssignmentClassMap : IEntityTypeConfiguration<AgentRoleAssignment>
{
    public void Configure(EntityTypeBuilder<AgentRoleAssignment> entity)
    {
        ...
        entity.HasDiscriminator(e => e.PrincipalDiscriminator)
            .HasValue<AgentRoleAssignment>(RoleAssignmentPrincipalDiscriminator.Base)
            .HasValue<UserAgentRoleAssignment>(RoleAssignmentPrincipalDiscriminator.User)
            .HasValue<ServicePrincipalAgentRoleAssignment>(RoleAssignmentPrincipalDiscriminator.ServicePrincipal)
            .HasValue<GroupAgentRoleAssignment>(RoleAssignmentPrincipalDiscriminator.Group);
        ...
        // Index on AgentId for efficient lookups
        entity
            .HasIndex(e => e.AgentId);

        // Composite index for permission query optimization
        entity
            .HasIndex(e => new { e.AgentId, e.OrganizationId, e.PrincipalId, e.RoleName })
            .HasDatabaseName("IX_AgentRoleAssign_Agent_Principal_Org_Role");

        // Organization navigation property
        entity
            .HasOne(e => e.Organization)
            .WithMany(x => x.AgentRoleAssignments)
            .HasForeignKey(e => e.OrganizationId)
            .OnDelete(DeleteBehavior.Restrict);

        // Agent navigation property
        entity
            .HasOne(e => e.Agent)
            .WithMany(x => x.AgentRoleAssignments)
